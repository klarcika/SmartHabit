...
// prod: /api   (isti origin na Vercelu)
...
function Habit() {
  ...
  const fetchHabits = async () => {
    ...
    try {
      if (isOnline) {
        const response = await axios.get(`${API}/api/habits`, {
  ...
  const handleAddHabit = async (e) => {
    ...
    const url = isEditing ? `${API}/api/habits/${currentHabit._id}` : `${API}/api/habits`;
    ...
    try {
      ...
      if (isOnline) {
        ...
        await queueRequest({ method, url, body: newHabitData });
  ...
  const handleDeleteHabit = async (habitId) => {
    ...
    const token = await getToken();
    const url = `${API}/api/habits/${habitId}`;
